...
function HttpUpload:doRequest(cmd,path,data)
    ...
    local function onReadyStateChanged()
        ...
        printInfo(string.format("cmd:%d  %s%d%s%d%s%s", cmd, "State:", xhr.readyState, " status:", xhr.status, " response:", xhr.response))
        this:doResponse(cmd,output,callback,succeed)
    ...
    if content and content~="" then
        ...
        this:doResponse(cmd,{code=-8,resean="file path is wrong"},callback)
    end
end

function HttpUpload:doResponse(cmd,data,callback,succeed)
